...
.encode.arg <- function(some.object, serialize.it = FALSE) {
    if (serialize.it) {
        encoded <- paste0(RCurl::base64Encode(jsonlite::serializeJSON(some.object)), 
                          "serialized")
    } else {
        encoded <- RCurl::base64Encode(jsonlite::toJSON(some.object, 
                                                        null = "null"))
    }
        ...
        tryCatch({
            ...
            invisible(mclapply(names(opals), mc.cores=1, function(opn) {
                ...
                tryCatch({
                    ...
                    #                       "', async=F)")
                    command.opn <- paste0("crossAggregate(mates, '", 
                                          .encode.arg(paste0("as.call(list(as.symbol('pushValue'), dsSSCP:::.encode.arg(crossProdSelf)))")), 
                                          "', async=F)")
                    cat("Command: ", command.opn, "\n")
