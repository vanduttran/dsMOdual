...
federateSSCP <- function(loginFD, logins, querytab, queryvar, TOL = 1e-10) {
    ...
    singularProdCross <- mclapply(singularProdCrossDSC, mc.cores=min(length(singularProdCrossDSC), detectCores()), function(dscbigmatrix) {
...
federateCov <- function(logins, querytab, queryvar, nameFD = NA) {
    ...
    print("OK")
    print(opals[nameFD])
    print(as.symbol(paste0('sumMatrices(crossProdSelf, ', .encode.arg(crossProdSelfDSC), ')')))
    DSI::datashield.assign(opals[nameFD], "crossProdAll", as.symbol(paste0('sumMatrices(crossProdSelf, ', .encode.arg(crossProdSelfDSC), ')')))
